...
On the held-out validation set the model picks the same move as the reference games **45.4%** of the time, up from the **17.8%** baseline. Validation accuracy (45.40%) even edges out training accuracy (44.90%), which suggests the model generalises well and isn't overfitting.
...
├── model.h5                                   # final trained model (Keras 3)
├── assets/
